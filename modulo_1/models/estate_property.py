...
class EstateProperty(models.Model):
    ...
    def _compute_best_offer(self):
        for record in self:
            record.best_offer = max(self.mapped("offer_ids.price"))
...
class EstatePropertyOffer(models.Model):
    ...
    create_date = fields.Integer(default=10)
    validity = fields.Integer(string="Validity", default=7)
    date_deadline = fields.Date(string="Deadline", compute="_computed_date_deadline")

    @api.depends("create_date")
    def _computed_date_deadline(self):
        for record in self:
            record.date_deadline = record.create_date +1
